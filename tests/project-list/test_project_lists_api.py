"""
project-list — ProjectListsApi HTTP integration tests.

10 tests covering CRUD operations and PDF generation.

Infrastructure: make infra-project-list-up
Run: PROJECT_LISTS_API_HOST=localhost:8086 pytest tests/project-list/ -v
"""
...
class TestProjectListsApi:
    ...
    def test_generate_specification_document_returns_pdf(self, project_lists_result):
        """
        Request PDF generation for SEEDED_LIST_ID using a real SKU (1039960000) that
        exists in the Firestore emulator after ETL data loading.  ProductsApi is called
        directly (no WireMock bridge).  WireMock still provides:
          - POST /oauth/token  → fake token (XMCloud auth)
          - POST /graphql      → empty dictionary items
        Requires: ETL data loaded via `make test-data-loader` (or `make load-etl-data`).
        """
        _, auth_session, _ = project_lists_result
        ...
        payload = {
                        ...
                        "items": [
                            {
                                ...
                                "quantity": 1,
                                "productUrl": "https://www.grohe.com/de-de/products/1039960000",
                            }
                        ],
                    }
                ],
            ...
            "specificationDocumentOptions": {
                ...
                "description": "Generated by project-list integration tests",
                "lang": "de-DE",
